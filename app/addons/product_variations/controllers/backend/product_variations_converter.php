...
function fn_product_variations_convert_process($by_variations, $by_combinations, $product_ids)
{
    ...
    fn_set_hook('variations_convert_process_post', $by_variations, $by_combinations, $product_ids, $counter, $errors);
...
function fn_product_variations_convert_process_configurable_product($parent_product_id, $children)
{
    ...
    $is_commerceml_installed = Registry::isExist('addons.rus_exim_1c');

    if ($is_commerceml_installed) {
        $base_fields[] = 'external_id';
        $base_fields[] = 'update_1c';
    }
...
function fn_product_variations_convert_process_product_with_combinations($product_id, $combinations)
{
    ...
    fn_set_hook('product_variations_convert_process_product_with_combinations_pre',
        $product_id,
        $combinations
    );
    ...
    fn_set_hook('product_variations_convert_process_product_with_combinations_after_prepare_data',
        $product_id,
        $combinations,
        $product_exceptions,
        $product_row,
    ...
    if (empty($combination_ids)) {
        $result = $service->createGroup([$product_id], null, $feature_collection);
    } else {
    ...
    if ($result->isSuccess()) {
        ...
        $group_products = $group->getProducts()->getProducts();
        ...
        fn_set_hook('product_variations_convert_process_product_with_combinations_post',
            $product_id,
            $combinations,
            $group_products,
            $product_repository,
...
function fn_product_variations_convert_process_feature(array $feature)
{
    ...
    fn_set_hook('product_variations_convert_process_feature_post', $feature);
...
function fn_product_variations_convert_get_products_using_combinations($filter_product_ids, $limit = 1000)
{
    ...
    do {
        ...
        fn_set_hook('product_variations_convert_get_products_using_combinations', $filter_product_ids, $limit, $products, $products);
...
function fn_product_variations_convert_find_usage_options($by_variations = true, $by_combinations = true, array $filter_product_ids = [])
{
    ...
    fn_set_hook('product_variations_convert_find_usage_options_post', $by_variations, $by_combinations, $filter_product_ids, $options);
...
function fn_product_variations_convert_get_features($options)
{
    ...
    fn_set_hook('product_variations_convert_get_features_post', $options, $result);
