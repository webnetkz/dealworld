...
class ProductFeatureImporter
{
    ...
    public function import(ProductFeatureDto $product_feature, ImportStorage $import_storage)
    {
        ...
        $product_feature_id = $import_storage->findEntityLocalId($product_feature);
        ...
        if (!$product_feature_id) {
            ...
            $product_feature->id->local_id = $product_feature_id;
        }
    ...
    {
        $main_result = new OperationResult(true);
        $allow_import_features = $import_storage->getSetting('allow_import_features', true);
        ...
        $product_feature_id = $import_storage->findEntityLocalIdByExternalId(
            ProductFeatureDto::REPRESENT_ENTITY_TYPE,
            $product_feature_value->feature_id->getId()
        );
        ...
        $product_feature_value->feature_id->local_id = $product_feature_id;
        $variant_id = null;

        if ($product_feature_value->value_id && !$product_feature_value->value_id->hasLocalId()) {
            $variant_id = $import_storage->findEntityLocalIdByExternalId(
                ProductFeatureVariantDto::REPRESENT_ENTITY_TYPE,
                $product_feature_value->value_id->getId()
            );
        }

        if ($variant_id && $product_feature_value->value_id) {
            $product_feature_value->value_id->local_id = $variant_id;
        } else {
    ...
    private function importProductFeatureVariants(OperationResult $main_result, ProductFeatureDto $product_feature, ImportStorage $import_storage)
    {
        ...
        foreach ($product_feature->variants as $variant) {
            ...
            $variant_id = $import_storage->findEntityLocalId($variant);

            if ($variant_id) {
                $variant->id->local_id = $variant_id;
