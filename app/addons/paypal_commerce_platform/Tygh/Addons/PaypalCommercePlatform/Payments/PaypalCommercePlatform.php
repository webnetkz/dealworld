...
class PaypalCommercePlatform
{
    ...
    /** @var array<int, string> $receivers_cache */
    protected static $receivers_cache = [];

    /** @var array<string, int|float|string|array> $order_info */
    protected $order_info = [];

    /**
     * @var array<string, string|int> $processor_params
     *
     * @psalm-var array{
     *   access_token: string,
     *   client_id: string,
    ...
    /** @var \Tygh\Addons\PaypalCommercePlatform\Api\ClientWrapper $api_requestor */
    protected $api;
    ...
    {
        $monetary_amount = $this->formatter->asPrice($amount, $this->processor_params['currency'], false, false);
        /** @var float $amount_formatted */
        $amount_formatted = fn_format_rate_value($monetary_amount, 'F', 2, '.', '');
    ...
    protected function buildPurchaseUnit(array $order, $fee)
    {
        ...
        $shipping_name = $this->buildShippingName($order);
    ...
    protected function buildProductItem(array $product)
    {
        ...
        $item = [
            'name'        => strip_tags($product['product']),
    ...
    protected function buildPurchaseUnitAmount(array $order)
    {
        ...
        /** @var array{value: float, currency_code: string} $subtotal */
        $subtotal = $this->formatAmount($this->getOrderSubtotal($order));
    ...
    {
        $total = $order['total'];
        $shipping = $this->getOrderShippingCost($order);
